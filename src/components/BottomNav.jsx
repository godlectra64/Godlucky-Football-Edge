...
export default function BottomNav({ activePage, onNavigate }) {
  return (
    <nav className="native-bottom-nav fixed inset-x-0 bottom-0 z-30 border-t border-white/10 bg-[rgba(3,6,9,0.92)] px-2 pt-2 shadow-[0_-18px_48px_rgba(0,0,0,0.5)] backdrop-blur-xl">
      <div className="mx-auto grid max-w-[480px] grid-cols-6 gap-1">
        {items.map(({ key, label, icon: Icon, className }) => {
          ...
          return (
            <button
              ...
              className={`premium-focus ${className} flex min-h-[52px] min-w-0 flex-col items-center justify-center gap-1 rounded-2xl border px-1 text-[10px] font-black leading-none transition duration-200 ${
                active
