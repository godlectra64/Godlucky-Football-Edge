...
export function buildDailySyncStartPayload(mode, state, durationMs, options = {}) {
  ...
  return {
    ...
    partial: Boolean(waitingRetry || state?.run?.status === 'partial'),
    continuation: nextStep?.continuation_state ?? waitingRetry?.continuation_state ?? null,
    processed: 0,
...
export function buildDailySyncStepResponseCached(mode, result, providerResult, durationMs, options = {}) {
  ...
  return {
    ...
    partial: Boolean(summary.partial || summary.status === 'pending_retry' || waitingRetry || result?.run?.status === 'partial'),
    continuation: summary.continuationState ?? result?.step?.continuation_state ?? waitingRetry?.continuation_state ?? null,
