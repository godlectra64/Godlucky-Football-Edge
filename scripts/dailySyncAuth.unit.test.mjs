...
assertIncludes(source, "'fixture-enrichment'", 'fixture-enrichment must be registered as a direct enrichment/admin mode')
assertIncludes(source, "code: 'UNKNOWN_SYNC_MODE'", 'unknown sync modes must return an explicit UNKNOWN_SYNC_MODE response')
assertIncludes(source, 'supportedModes: getSupportedSyncModes()', 'unknown sync mode response must expose supported modes')
assertIncludes(source, 'requestedMode,', 'sync responses must expose requestedMode for routing debug')
assertIncludes(source, 'resolvedMode: mode', 'sync responses must expose resolvedMode for routing debug')
assertIncludes(source, "if (mode === 'fixture-enrichment')", 'fixture-enrichment direct mode must use a dedicated route')
assertIncludes(source, "executeDailySyncPhase(2, 'fixture-enrichment'", 'fixture-enrichment direct mode must reuse the daily fixture-enrichment phase logic')
